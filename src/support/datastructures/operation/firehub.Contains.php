...
use FireHub\Core\Support\DataStructures\Contracts\ArrayableStorage;
...
readonly class Contains {
     ...
     *
     * new Contains($collection)->key('firstname');
     *
     * // true
     * ```
     *
     ...
     */
    public function key (mixed $key):bool {

        if ($this->data_structure instanceof ArrayableStorage && (DataIs::int($key) || DataIs::string($key)))
            return Arr::keyExist($key, $this->data_structure->toArray());
     ...
     * $collection = new Associative(['firstname' => 'John', 'lastname' => 'Doe', 'age' => 25, 10 => 2]);
     *
     * new Contains($collection)->value('John');
     *
     * // true
     * ```
     *
    ...
    public function value (mixed $value):bool {
        ...
        if ($this->data_structure instanceof ArrayableStorage)
            return Arr::inArray($value, $this->data_structure->toArray());

        foreach ($this->data_structure as $storage_value)
            if ($value === $storage_value) return true;
     ...
     * $collection = new Associative(['firstname' => 'John', 'lastname' => 'Doe', 'age' => 25, 10 => 2]);
     *
     * new Contains($collection)->where(fn($value, $key) => $value === 'John');
     *
     * // true
     * ```
     *
